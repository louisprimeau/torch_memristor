        ...
        for i in range(self.size[0] // self.tile_rows):
            for j in range(self.size[1] // self.tile_cols):
                for k in range(voltage.size(1)):
                    tile = self.W[i*self.tile_rows:(i+1)*self.tile_rows, j*self.tile_cols:(j+1)*self.tile_rows]
                    vect = voltage[i*self.tile_rows:(i+1)*self.tile_rows,k]
                    solution  = self.circuit_solve(tile, vect, torch.zeros(self.size[1]), torch.ones(self.size[1]), torch.zeros(self.size[0]))
